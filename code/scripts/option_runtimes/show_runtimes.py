...
model = LinearRegression().fit(
    np.log(df_end.N.values.reshape([-1, 1])), np.log(df_end.Time.values)
)
    ...
    for linkage in df.Linkage.unique():
        ...
        for covariance_type in df.Covariance_Type.unique():
            N = df.loc[
                ...
                & (df["Linkage"] == linkage)
                & (df["Covariance_Type"] == covariance_type)
            ]["N"]
            T = df.loc[
                (df["Affinity"] == affinity)
                & (df["Linkage"] == linkage)
...
plt.plot(
    x, y, color="green", label="GraSPyclust slope: %1.2f" % graspyslope, **kwargs_reg
)
...
plt.title(
    "Runtimes of All Clustering Options \n on Different Sized Datasets",
    fontsize=24,
    fontweight="bold",
)
