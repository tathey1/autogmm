...
if p < 0.05:
    ...
    plt.text((x1 + x2) * 0.5, y + h - 0.5 * h, txt, ha="center", va="bottom", color=col)
    plt.ylim((0, 1.2))
...
if p < 0.05:
    ...
    plt.text((x1 + x2) * 0.5, h_txt, txt, ha="center", va="bottom", color=col)
...
if p < 0.05:
    ...
    plt.plot(
        [x1, x1, x2, x2],
        [bottom_high * 3, top * 3, top * 3, bottom_low * 3],
        lw=1.5,
        c=col,
    )
    plt.text((x1 + x2) * 0.5, h_txt * 4, txt, ha="center", va="bottom", color=col)
...
if p < 0.05:
    ...
    plt.text((x1 + x2) * 0.5, h_txt * 3, txt, ha="center", va="bottom", color=col)
...
plt.yscale("log")
plt.suptitle(
    "Clustering Runtimes on Different Datasets", fontweight="bold", fontsize=18
)
plt.xticks(fontsize=13)
