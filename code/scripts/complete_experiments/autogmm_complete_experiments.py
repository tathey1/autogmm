...
if dataset == 0:
    ...
    c_true = np.genfromtxt(
        "../../../data/synthetic.csv", delimiter=",", usecols=(0), skip_header=0
    )
elif dataset == 1:
    ...
    x = np.genfromtxt(
        "../../../data/wdbc.data", delimiter=",", usecols=(3, 25, 26), skip_header=0
    )
...
def make_bic_plots(results, best_cov, best_k_bic, best_bic):
    ...
    for i, cov_type in enumerate(cov_types):
        bics[i * 11 + 0, :] = -results.loc[
            (results["covariance_type"] == cov_type) & (results["affinity"] == "none")
        ]["bic/aic"].values
        bics[i * 11 + 1, :] = -results.loc[
            (results["covariance_type"] == cov_type)
            & (results["affinity"] == "euclidean")
            & (results["linkage"] == "ward")
        ]["bic/aic"].values
        bics[i * 11 + 2, :] = -results.loc[
            (results["covariance_type"] == cov_type)
            & (results["affinity"] == "euclidean")
            & (results["linkage"] == "complete")
        ]["bic/aic"].values
        bics[i * 11 + 3, :] = -results.loc[
            (results["covariance_type"] == cov_type)
            & (results["affinity"] == "euclidean")
            & (results["linkage"] == "average")
        ]["bic/aic"].values
        bics[i * 11 + 4, :] = -results.loc[
            (results["covariance_type"] == cov_type)
            & (results["affinity"] == "euclidean")
            & (results["linkage"] == "single")
        ]["bic/aic"].values
        bics[i * 11 + 5, :] = -results.loc[
            (results["covariance_type"] == cov_type)
            & (results["affinity"] == "manhattan")
            & (results["linkage"] == "complete")
        ]["bic/aic"].values
        bics[i * 11 + 6, :] = -results.loc[
            (results["covariance_type"] == cov_type)
            & (results["affinity"] == "manhattan")
            & (results["linkage"] == "average")
        ]["bic/aic"].values
        bics[i * 11 + 7, :] = -results.loc[
            (results["covariance_type"] == cov_type)
            & (results["affinity"] == "manhattan")
            & (results["linkage"] == "single")
        ]["bic/aic"].values
        bics[i * 11 + 8, :] = -results.loc[
            (results["covariance_type"] == cov_type)
            & (results["affinity"] == "cosine")
            & (results["linkage"] == "complete")
        ]["bic/aic"].values
        bics[i * 11 + 9, :] = -results.loc[
            (results["covariance_type"] == cov_type)
            & (results["affinity"] == "cosine")
            & (results["linkage"] == "average")
        ]["bic/aic"].values
        bics[i * 11 + 10, :] = -results.loc[
            (results["covariance_type"] == cov_type)
            & (results["affinity"] == "cosine")
            & (results["linkage"] == "single")
        ]["bic/aic"].values
    ...
    fig, ((ax0, ax1), (ax2, ax3)) = plt.subplots(
        2, 2, sharey="row", sharex="col", figsize=(7, 6)
    )
    ...
    fig.text(
        0.5, 0.04, "Number of Components", ha="center", fontsize=18, fontweight="bold"
    )
    fig.text(
        0.01,
        0.5,
...
def make_cluster_plots(x, c_true, c_hat_autogmm):
    ...
    plt.scatter(
        x[:, 0], x[:, 1], c=c_true, cmap=colors.ListedColormap(c_list[0 : max_c + 1])
    )
    ...
    plt.scatter(
        ...
        x[:, 1],
        c=c_hat_autogmm,
        cmap=colors.ListedColormap(c_list[0 : max_c + 1]),
    )
    plt.xlabel("First Dimension", fontsize=24)
    plt.ylabel("Second Dimension", fontsize=24)
